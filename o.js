(function () {
		...
		window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
			if (!window.rch_nws[hostkey].startTypeInvoke) {
				...
				if (Lampa.Platform.is("android") || Lampa.Platform.is("tizen")) check(true);
					...
					var net = new Lampa.Reguest();
					net.silent(
						Config.Urls.LampaVip.indexOf(location.host) >= 0 ? Config.Urls.GithubCheck : host + Config.Urls.CorsCheckPath,
						function () {
							check(true);
						},
						function () {
	...
	if (typeof window.rch_nws[hostkey].Registry !== "function")
		window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
			new Promise(function (resolve) {
				window.rch_nws[hostkey].typeInvoke(Config.Urls.LampaVip, resolve);
			})
				.then(function () {
					client.invoke(
						"RchRegistry",
	...
	function component(object) {
		...
		this.initialize = function () {
			...
			function onFilterSelectItem(a, b) {
				...
				select_close_timer = setTimeout(Lampa.Select.close, 10);
			}
	...
	function initBalanserInFilterMenu() {
		if (window.lampavip_src_filter_plugin) {
			return;
		}

		window.lampavip_src_filter_plugin = true;
		...
		Lampa.Controller.listener.follow("toggle", function (event) {
			...
			if (!active || !active.component || active.component.toLowerCase() !== "lampavip") {
				return;
			}
			...
			if ($(".selectbox-item[data-lampavip-source]").length > 0) {
				return;
			}
			...
			$selectBoxItem.attr("data-lampavip-source", "true");
